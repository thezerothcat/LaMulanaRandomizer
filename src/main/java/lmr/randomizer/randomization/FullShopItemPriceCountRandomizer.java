 ...
 */
public class FullShopItemPriceCountRandomizer implements ShopItemPriceCountRandomizer {
...
//            "Provocative Bathing Suit", "Spaulder", "Flail Whip", "lamulana.exe");
    ...
    public FullShopItemPriceCountRandomizer(boolean subweaponOnly, MoneyChecker moneyChecker, Random random) {
    ...
    private short getPrice(String itemName, String shopInventoryLocation) {
        ...
        Short price = getWeightPrice(itemName);
        if(price != null) {
            return price;
        }
        price = getAmmoPrice(itemName);
        if(price != null) {
            return price;
        }
