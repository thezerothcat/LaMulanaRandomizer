        ...
        if(Settings.isRandomizeTrapItems()) {
            if (screen.getZoneIndex() == 5 && screen.getRoomIndex() == 1 && screen.getScreenIndex() == 1) {
                if(!Settings.isFools2020Mode()) {
                    for(TestByteOperation testByteOperation : graphicsTextureDraw.getTestByteOperations()) {
                        if(testByteOperation.getIndex() == FlagConstants.INFERNO_FAKE_ORB_CRUSHER) {
                            ...
                            graphicsTextureDraw.setId((short)0x2f);
                            graphicsTextureDraw.getArgs().clear();
                            graphicsTextureDraw.getArgs().add((short)0); // Interactable any time?
                            graphicsTextureDraw.getArgs().add((short)69); // Sacred Orb item
                            graphicsTextureDraw.getArgs().add((short)0); // Fake item
                            graphicsTextureDraw.getTestByteOperations().get(0).setIndex(FlagConstants.WF_TRAP_INFERNO);

                            graphicsTextureDraw.getWriteByteOperations().add(new WriteByteOperation(FlagConstants.WF_TRAP_INFERNO, ByteOp.ASSIGN_FLAG, 1));
                            break;
                        }
                    }
                }
                ...
                if(!graphicsTextureDraw.getTestByteOperations().isEmpty() && graphicsTextureDraw.getTestByteOperations().get(0).getIndex() == FlagConstants.ROOM_FLAG_35) {
                    ...
                    graphicsTextureDraw.getArgs().clear();
                    graphicsTextureDraw.getArgs().add((short)0); // Interactable any time?
                    graphicsTextureDraw.getArgs().add((short)19); // Ankh Jewel item
                    graphicsTextureDraw.getArgs().add((short)0); // Fake item
                    graphicsTextureDraw.getTestByteOperations().get(0).setIndex(FlagConstants.WF_TRAP_TWIN);

                    WriteByteOperation writeByteOperation = new WriteByteOperation(FlagConstants.WF_TRAP_TWIN, ByteOp.ASSIGN_FLAG, 1);
                    graphicsTextureDraw.getWriteByteOperations().add(writeByteOperation);
                }
            }
        }
...
//                        doorName = "Door: B7";
    ...
    boolean updateConversationDoor(GameObject conversationDoor) {
        ...
        else if(conversationDoor.getArgs().get(4) == 676 && Settings.isRandomizeNpcs()) {
            // Giltoriyo, Alsedana, Samaranta, Fobos conversations without Philosopher's Ocarina
            return false;
        }
        else if(conversationDoor.getArgs().get(4) == 682 && Settings.isRandomizeNpcs()) {
        ...
        else if(conversationDoor.getArgs().get(4) == 685 && Settings.isRandomizeNpcs()) {
            // To be removed and re-added if shuffling NPCs for simplicity.
            return false;
        }
        else if(conversationDoor.getArgs().get(4) == 687 && Settings.isRandomizeNpcs()) {
            // Fairy Queen conversation, to be removed and re-added if shuffling NPCs for simplicity.
            return false;
        }
        else if(conversationDoor.getArgs().get(4) == 688 && Settings.isRandomizeNpcs()) {
            // Fairy Queen conversation, to be removed and re-added if shuffling NPCs for simplicity.
            return false;
        }
        ...
        else if(conversationDoor.getArgs().get(4) == 985 && (Settings.isRandomizeNpcs())) {
            // To be removed and re-added if shuffling NPCs for simplicity.
            return false;
        }
...
//                        gameObject.getArgs().get(3), gameObject.getArgs().get(4)));
    ...
    private void updateStartingScreen() {
        ...
        addStartingShop(screen, zoneIndex);
